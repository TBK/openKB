...
if (config.settings.theme) {
  if (!fs.existsSync(path.join(__dirname, '/public/themes/', config.settings.theme))) {
    console.error(
      'Theme folder does not exist. Please check theme in /routes/config.js');
    process.exit();
  }
}
    ...
    split_keywords: function(keywords) {
      ...
      if (keywords) {
        ...
        for (var i = 0; i < array.length; i++) {
          if (array[i].trim() !== '') {
            links += '<a href="' + app_context + '/search/' + array[i].trim() +
              '">' + array[i].trim() + '</a>&nbsp;|&nbsp;';
          }
        }
...
app.use(app_context + '/font-awesome', express.static(path.join(__dirname,
  'node_modules/font-awesome/')));
app.use(app_context + '/jquery', express.static(path.join(__dirname,
  'node_modules/jquery/dist/')));
app.use(app_context + '/bootstrap', express.static(path.join(__dirname,
  'node_modules/bootstrap/dist/')));
app.use(app_context + '/bootstrapTabs', express.static(path.join(__dirname,
  'node_modules/bootstrap/js/')));
app.use(app_context + '/simplemde', express.static(path.join(__dirname,
  'node_modules/simplemde/dist/')));
app.use(app_context + '/markdown-it', express.static(path.join(__dirname,
  'node_modules/markdown-it/dist/')));
app.use(app_context + '/stylesheets', express.static(path.join(__dirname,
  'public/stylesheets')));
app.use(app_context + '/fonts', express.static(path.join(__dirname,
  'public/fonts')));
app.use(app_context + '/javascripts', express.static(path.join(__dirname,
  'public/javascripts')));
app.use(app_context + '/lunr', express.static(path.join(__dirname,
  'node_modules/lunr')));
app.use(app_context + '/favicon.png', express.static(path.join(__dirname,
  'public/favicon.png')));
...
if (config.settings.database.type === 'embedded') {
  ...
  indexArticles(db, function(err) {
    ...
    app.listen(app.get('port'), app.get('bind'), function() {
      console.log('openKB running on host: http://' + app.get('bind') +
        ':' + app.get('port'));
      app.emit('openKBstarted');
    });
  });
} else {
  MongoClient.connect(config.settings.database.connection_string, {}, function(
    err, db) {
    ...
    indexArticles(db, function(err) {
      // lift the app
      app.listen(app.get('port'), app.get('bind'), function() {
        console.log('openKB running on host: http://' + app.get(
          'bind') + ':' + app.get('port'));
        app.emit('openKBstarted');
      });
    });
...
function indexArticles(db, callback) {
  ...
  if (config.settings.database.type === 'embedded') {
    ...
    db.kb.find({
      kb_published: 'true'
    }).toArray(function(err, kb_list) {
      // add to lunr index
      kb_list.forEach(function(kb) {
        // only if defined
        var keywords = '';
        if (typeof kb.kb_keywords !== 'undefined' && kb.kb_keywords !==
          null) {
          keywords = kb.kb_keywords.toString().replace(/,/g, ' ');
        }
