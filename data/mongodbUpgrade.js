...
mongodb.connect(config.settings.database.connection_string, {}, function(err,
  mdb) {
  if (err) {
...
function insertKB(db, callback) {
  ...
  ndb.loadDatabase(function(err) {
    ...
    ndb.find({}, function(err, docs) {
      if (docs.length === 0) {
        console.error(
          'The NeDB database contains no data, no work required');
        console.error(
          'You should probably check the NeDB datafile path though!');
      } else {
        console.log('Loaded ' + docs.length +
          ' article(s) data from the NeDB database');
        console.log('');
      }
  ...
  ndb.loadDatabase(function(err) {
    ...
    ndb.find({}, function(err, docs) {
      if (docs.length === 0) {
        ...
        console.log('Loaded ' + docs.length +
          ' user(s) data from the NeDB database');
        console.log('');
      }
      ...
      async.each(docs, function(doc, cb) {
        ...
        if (err) {
          console.error(
            'An error happened while inserting user data');
          callback(err, null);
        } else {
