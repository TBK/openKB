...
router.get('/', common.restrict, function(req, res, next) {
  ...
  var featuredCount = config.settings.featured_articles_count ? config.settings
  ...
  var sortByField = typeof config.settings.sort_by.field !== 'undefined' ?
    config.settings.sort_by.field : 'kb_viewcount';
  var sortByOrder = typeof config.settings.sort_by.order !== 'undefined' ?
    config.settings.sort_by.order : -1;
  var sortBy = {};
  ...
  common.dbQuery(db.kb, {
    ...
    common.dbQuery(db.kb, {
      ...
      res.render('index', {
        ...
        session: req.session,
        message: common.clear_session_value(req.session,
          'message'),
        message_type: common.clear_session_value(req.session,
          'message_type'),
        config: config,
...
router.get('/' + config.settings.route_name + '/:id/version', common.restrict,
  function(req, res) {
    var db = req.app.db;
    common.config_expose(req.app);
...
router.get('/' + config.settings.route_name + '/:id', common.restrict, function(
  req, res) {
  var db = req.app.db;
  common.config_expose(req.app);
  var classy = require('../public/javascripts/markdown-it-classy');
  ...
  db.kb.findOne({
    ...
    if (result === null || result.kb_published === 'false') {
      ...
      if (result.kb_password) {
        if (result.kb_password !== '') {
          if (req.session.pw_validated === 'false' || req.session.pw_validated ===
            undefined || req.session.pw_validated === null) {
      ...
      db.kb.update({
        ...
        common.dbQuery(db.kb, {
          kb_published: 'true'
        }, sortBy, featuredCount, function(err,
          featured_results) {
          res.render('kb', {
            title: result.kb_title,
            result: result,
            ...
            config: config,
            session: req.session,
            current_url: req.protocol + '://' + req.get(
              'host') + req.app_context,
...
router.post('/update_settings', common.restrict, function(req, res) {
  ...
  fs.writeFileSync(path.join(__dirname, 'config.js'), JSON.stringify(config,
    null, 4), 'utf8');
...
router.get('/' + config.settings.route_name + '/resetviewCount/:id', common.restrict,
  function(req, res) {
    var db = req.app.db;
    db.kb.update({
      _id: common.getId(req.params.id)
      ...
      if (err) {
        req.session.message = req.i18n.__(
          'View count could not be reset. Try again.');
        req.session.message_type = 'danger';
      } else {
        req.session.message = req.i18n.__(
          'View count successfully reset to zero.');
        req.session.message_type = 'success';
      }
...
router.get('/' + config.settings.route_name + '/resetvoteCount/:id', common.restrict,
  function(req, res) {
    var db = req.app.db;
    db.kb.update({
      _id: common.getId(req.params.id)
      ...
      if (err) {
        req.session.message = req.i18n.__(
          'Vote count could not be reset. Try again.');
        req.session.message_type = 'danger';
      } else {
        req.session.message = req.i18n.__(
          'Vote count successfully reset to zero.');
        req.session.message_type = 'success';
      }
      ...
      db.kb.insert(doc, function(err, newDoc) {
        if (err) {
          ...
          if (req.body.frm_kb_keywords !== undefined) {
            keywords = req.body.frm_kb_keywords.toString().replace(
              /,/g, ' ');
          }
          ...
          // add to store
          var href = req.body.frm_kb_permalink !== '' ? req.body.frm_kb_permalink :
            newId;
...
router.post('/insert_suggest', common.suggest_allowed, function(req, res) {
  ...
  db.kb.insert(doc, function(err, newDoc) {
    if (err) {
      ...
      req.session.message = req.i18n.__(
        'Suggestion failed. Please contact admin.');
      req.session.message_type = 'danger';
      ...
      // redirect to new doc
      req.session.message = req.i18n.__(
        'Suggestion successfully processed');
      req.session.message_type = 'success';
      res.redirect(req.app_context + '/');
    }
  });
  ...
  db.kb.count({
    ...
    if (kb > 0 && req.body.frm_kb_permalink !== '') {
      ...
      db.kb.findOne({
        ...
        var author = article.kb_author ? article.kb_author : req.session
          .users_name;
        var author_email = article.kb_author_email ? article.kb_author_email :
          req.session.user;

        // set published date to now if none exists
        var published_date;
        if (article.kb_published_date === null || article.kb_published_date ===
          undefined) {
        ...
        db.kb.update({
          ...
          $set: {
            ...
            kb_keywords: keywords,
            kb_last_updated: new Date(),
            kb_last_update_user: req.session.users_name + ' - ' +
              req.session.user,
            kb_author: author,
          ...
          if (err) {
            ...
            // update store
            var href = req.body.frm_kb_permalink !== '' ? req.body
              .frm_kb_permalink : req.body.frm_kb_id;
            ...
            var article_versioning = config.settings.article_versioning ?
              config.settings.article_versioning : false;
            ...
            if (article_versioning === true) {
              ...
              db.kb.insert(version_doc, function(err,
                version_doc) {
                req.session.message = req.i18n.__(
                  'Successfully saved');
                req.session.message_type = 'success';
                res.redirect(req.app_context + '/edit/' +
                  req.body.frm_kb_id);
              });
            } else {
              req.session.message = req.i18n.__(
                'Successfully saved');
              req.session.message_type = 'success';
              res.redirect(req.app_context + '/edit/' + req.body
                .frm_kb_id);
            }
          }
        });
...
router.get('/user/edit/:id', common.restrict, function(req, res) {
  ...
  db.users.findOne({
    ...
    if (user.user_email !== req.session.user && req.session.is_admin ===
      'false') {
...
router.post('/user_insert', common.restrict, function(req, res) {
  ...
  db.users.findOne({
    ...
    if (user) {
      ...
      console.error('Failed to insert user, possibly already exists: ' +
        err);
      req.session.message = req.i18n.__(
    ...
    if (user === undefined || user === null) {
      ...
      if (bcrypt.compareSync(req.body.password, user.user_password) ===
        ...
        if (req.body.frm_referring_url === undefined || req.body.frm_referring_url ===
          ...
          if (url_parts.pathname !== '/setup' && url_parts.pathname !==
            '/login') {
...
router.post('/file/upload_file', common.restrict, inline_upload.single('file'),
...
router.post('/file/new_dir', common.restrict, function(req, res, next) {
  ...
  if (req.body.custom_dir) {
    mkdirp(path.join(appDir, 'public', 'uploads', req.body.custom_dir),
      function(err) {
        if (err) {
          console.error('Directory creation error: ' + err);
          req.session.message = req.i18n.__(
            'Directory creation error. Please try again');
          req.session.message_type = 'danger';
...
router.post('/file/upload', common.restrict, upload.single('upload_file'),
  function(req, res, next) {
    var fs = require('fs');
    ...
    if (req.file) {
      ...
      req.session.message = req.i18n.__(
        'File upload error. Please select a file.');
      req.session.message_type = 'danger';
      res.redirect(req.app_context + '/files');
    }
  });
...
router.get('/insert', common.restrict, function(req, res) {
  res.render('insert', {
    ...
    kb_permalink: common.clear_session_value(req.session,
      'kb_permalink'),
    message: common.clear_session_value(req.session, 'message'),
    message_type: common.clear_session_value(req.session,
      'message_type'),
...
router.post('/importer', common.restrict, upload.single('import_file'),
  function(req, res, next) {
    var fs = require('fs');
    var path = require('path');
    ...
    if (checkMime === false) {
      ...
      res.end(
        'File type not permitted. Please upload a zip of Markdown documents.'
      );
      return;
    }
    ...
    zipExtract(file.path, {
      ...
      fs.readdir(path.join(__dirname, '..', 'public', 'temp', 'import'), (
        err, files) => {
        files.forEach(file => {
          ...
          var faq_body = fs.readFileSync(path.join(__dirname,
            '..', 'public', 'temp', 'import', file), 'utf-8');
          if (faq_body === '') {
            faq_body = 'FAQ body';
          }
  ...
  common.dbQuery(db.kb, {}, null, null, function(err, results) {
    ...
    fs.writeFile('data/export.zip', buffer, function(err) {
      ...
      res.set('Content-Disposition',
        'attachment; filename=data/export.zip');
  ...
  common.dbQuery(db.kb, {
    ...
    for (var key in articles) {
      if (Object.prototype.hasOwnProperty.call(articles, key)) {
        // check for permalink
        var pageUrl = '/' + config.settings.route_name + '/' + articles[
          key]._id;
        if (articles[key].kb_permalink !== '') {
          pageUrl = '/' + config.settings.route_name + '/' + articles[
            key].kb_permalink;
        }
        urlArray.push({
          url: pageUrl,
